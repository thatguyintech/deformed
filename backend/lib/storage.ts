...
const web3StorageClient = new Web3Storage({ token: Env.getWeb3StorageApiToken() });
...
export async function retrieve(cid: string, fileName: string) {
    ...
    if (!res) {
        throw new InternalServerError(`Failed to get response when retrieving cid ${cid}.`)
    }
        
    if (!res.ok) {
        throw new InternalServerError(`Failed to get cid ${cid} - [${res.status}] ${res.statusText}`)
    }

    apiLogger.debug(res, `Response from web3.storage retrieval [${res.status}] ${res.statusText}`)
